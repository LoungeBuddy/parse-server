  ...
  _createClass(UserController, [{
    ...
    value: function verifyEmail(username, token) {
      ...
      return checkIfAlreadyVerified.execute().then(function (result) {
        ...
        return self.config.database.update('_User', query, updateFields).then(function (document) {
          if (!document) {
            throw undefined;
          }
    ...
    value: function resendVerificationEmail(username) {
      var self = this;
      return self.getUserIfNeeded({ username: username }).then(function (aUser) {
        if (!aUser || aUser.emailVerified) {
          throw undefined;
        }
        self.setEmailVerifyToken(aUser);
        return self.config.database.update('_User', { username: username }, aUser).then(function (res) {
